...
function rectangularArea(height, weight) {
...
function writeParagraf(someString) {
    document.write(`<div> <p>${someString}</p> </div>`);
}
...
function createUl_2(someString, ul_lenght){
    document.write(`<div><ul>`);
...
function writeMinNumberOfArray(array) {
    let minItem = array[0];
...
function curExchange(sum, currencyValues, exchangeCurrency) {
